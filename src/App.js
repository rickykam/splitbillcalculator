  ...
  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col">
      <div className="flex justify-between items-center p-4 z-20 bg-gradient-to-b from-black/60 to-transparent absolute top-0 left-0 right-0">
        <button
          onClick={onClose}
          className="text-white p-2 rounded-full bg-white/20 backdrop-blur-md hover:bg-white/30"
        >
          <X className="w-6 h-6" />
        </button>
      ...
      <div className="flex-1 relative bg-gray-900 overflow-hidden flex items-center justify-center">
        {status === "idle" && (
          <div className="flex flex-col items-center justify-center z-30 p-6 text-center">
            ...
            <button
              onClick={startCamera}
              className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-full font-bold shadow-xl active:scale-95 transition-all flex items-center gap-2"
            >
        ...
        {status === "error" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-20 px-6 text-center bg-black/90">
            ...
            <button
              onClick={onClose}
              className="px-6 py-2 bg-gray-700 text-white rounded-lg text-sm"
            >
              {t("close")}
            </button>
          </div>
        ...
        <video
          ...
          muted
          autoPlay
          className={`absolute min-w-full min-h-full object-cover transition-opacity duration-500 z-0 ${
            isReady && status === "streaming" ? "opacity-100" : "opacity-0"
          }`}
        />
        {isReady && status === "streaming" && (
          <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-10">
            <div className="w-[75%] h-[75%] border-2 border-white/40 rounded-lg relative shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]">
              ...
              <div className="absolute -top-10 left-0 right-0 text-center text-white/90 text-xs bg-black/50 py-1 rounded-full">
                {t("alignReceipt")}
              </div>
            </div>
          </div>
      ...
      {isReady && status === "streaming" && (
        <div className="bg-black pb-safe pt-6 pb-8 flex justify-center z-20">
          <button
            ...
            className="w-20 h-20 rounded-full border-4 border-white bg-white/10 active:bg-white/30 active:scale-95 transition-all flex items-center justify-center cursor-pointer"
          >
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex flex-col justify-end sm:justify-center animate-in fade-in duration-200">
      <div className="bg-white w-full h-full sm:h-[90%] sm:max-w-md sm:mx-auto sm:rounded-t-2xl flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4">
            ...
            {image ? (
              ...
              <div className="w-full flex gap-3">
                <button
                  ...
                  className="flex-1 py-4 bg-blue-50 border border-blue-100 rounded-xl flex flex-col items-center justify-center gap-2 hover:bg-blue-100 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-gray-50 border border-gray-200 rounded-xl flex flex-col items-center justify-center gap-2 hover:bg-gray-100 transition-colors"
                >
          ...
          {/* Name */}
          <div
            className={`bg-white p-4 rounded-xl shadow-sm mb-4 border ${
          ...
          >
            ...
            <input
              ...
              className="w-full text-xl font-medium border-b border-gray-200 focus:outline-none focus:border-blue-500 py-2"
            />
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm mb-4 border border-gray-100">
            <div className="flex justify-between items-center mb-3">
              ...
              {!showInlineAddUser ? (
                <button
                  onClick={() => setShowInlineAddUser(true)}
                  className="text-xs text-blue-600 font-bold flex items-center gap-1 bg-blue-50 px-2 py-1 rounded hover:bg-blue-100"
                >
                  <Plus className="w-3 h-3" /> {t("newMember")}
                </button>
              ) : (
                <div className="flex items-center gap-2 animate-in fade-in duration-200">
                  <input
                    className="w-24 text-sm border border-blue-300 rounded px-2 py-1 outline-none"
                  ...
                  />
                  <button
                    onClick={handleInlineAddUserSubmit}
                    className="text-green-600 bg-green-50 p-1 rounded hover:bg-green-100"
                  >
                    <Check className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setShowInlineAddUser(false)}
                    className="text-red-500 bg-red-50 p-1 rounded hover:bg-red-100"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            <div className="space-y-3">
              {users.map((user) => (
                <div key={user} className="flex items-center justify-between">
                  <span className="text-gray-700 font-medium w-1/3 truncate text-sm">
                    {user}
                  </span>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm mb-20 border border-gray-100">
            <div className="flex justify-between items-center mb-3">
              ...
              <button
                onClick={() =>
                  setSplitAmong(
                    splitAmong.length === users.length ? [] : [...users]
                  )
                }
          ...
          {initialData ? (
            <>
              <button
                ...
                className="flex-1 bg-red-50 text-red-500 py-3 rounded-xl font-bold text-lg shadow-sm active:scale-95 transition-transform flex items-center justify-center gap-2 border border-red-100 hover:bg-red-100"
              >
                <Trash2 className="w-5 h-5" /> {t("delete")}
              </button>
              <button
                onClick={() => handleSave(true)}
                className="flex-[2] bg-blue-600 text-white py-3 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2 hover:bg-blue-700"
              >
                <Check className="w-5 h-5" /> {t("saveChanges")}
              </button>
            </>
          ) : (
            <>
              <button
                ...
                className="flex-1 bg-blue-50 text-blue-600 py-3 rounded-xl font-bold text-lg shadow-sm active:scale-95 transition-transform flex items-center justify-center gap-2 border border-blue-100 hover:bg-blue-100"
              >
                <Plus className="w-5 h-5" /> {t("continueAdd")}
              </button>
              <button
                onClick={() => handleSave(true)}
                className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2 hover:bg-blue-700"
              >
                <Check className="w-5 h-5" /> {t("complete")}
              </button>
            </>
          )}
        </div>
...
const SplitBillApp = () => {
  ...
  // Persist state - No Example Data, Updated Default Users
  const [users, setUsers] = useState(
    () => JSON.parse(localStorage.getItem("users")) || ["Member A", "Member B"]
  );
  const [payIds, setPayIds] = useState(
    () => JSON.parse(localStorage.getItem("payIds")) || {}
  );
  ...
  useEffect(() => {
    localStorage.setItem("payIds", JSON.stringify(payIds));
  }, [payIds]);
  useEffect(() => {
  ...
  const handleSaveExpense = (data, shouldClose) => {
    ...
    if (editingExpenseId) {
      setExpenses((prev) =>
        prev.map((e) =>
          e.id === editingExpenseId ? { ...e, ...expensePayload } : e
        )
      );
    } else {
  ...
  const saveEditUser = () => {
    const oldN = editingUser;
    const newN = editUserNameInput.trim();
    if (!newN || newN === oldN) {
      if (newN) setPayIds({ ...payIds, [newN]: editUserPayIdInput });
      setEditingUser(null);
      return;
    }
    if (users.includes(newN)) {
      setUserAlertMsg(t("errNameExist"));
      return;
    }
    setUsers(users.map((u) => (u === oldN ? newN : u)));
    const newP = { ...payIds };
    delete newP[oldN];
    newP[newN] = editUserPayIdInput;
    setPayIds(newP);
    setExpenses((prev) =>
      prev.map((e) => {
        const pb = { ...e.paidBy };
  ...
  const removeUser = (u) => {
    if (expenses.some((e) => e.paidBy[u] > 0 || e.splitAmong.includes(u))) {
      setUserAlertMsg(t("userInUse"));
      return;
    }
  ...
  const openAddGroup = () => {
    ...
    setGroupTitleInput("");
    setGroupDateInput(new Date().toISOString().split("T")[0]);
    setIsGroupModalOpen(true);
  };
  const openEditGroup = (g) => {
    setEditingGroupId(g.id);
    setGroupTitleInput(g.title);
    setGroupDateInput(new Date(g.date).toISOString().split("T")[0]);
    setIsGroupModalOpen(true);
  };

  const saveGroup = () => {
    if (!groupTitleInput.trim()) {
      setUserAlertMsg(t("errGroupInput"));
      return;
    }
    ...
    if (editingGroupId)
      setGroups((prev) =>
        prev.map((g) =>
          g.id === editingGroupId
            ? { ...g, title: groupTitleInput, date: d }
            : g
        )
      );
    else {
      const g = { id: Date.now(), title: groupTitleInput, date: d };
      setGroups([g, ...groups]);
  ...
  const report = useMemo(() => {
    const stats = {};
    users.forEach((u) => (stats[u] = { paid: 0, share: 0, net: 0 }));
    let totalSpending = 0;
    currentGroupExpenses.forEach((e) => {
      ...
      Object.entries(e.paidBy).forEach(([u, amt]) => {
        if (stats[u]) {
          stats[u].paid += amt;
          stats[u].net += amt;
        }
    ...
    while (i < d.length && j < c.length) {
      ...
      tx.push({
        ...
        payId: payIds[c[j].user],
      });
      d[i].val += amt;
      c[j].val -= amt;
      if (Math.abs(d[i].val) < 0.01) i++;
      if (Math.abs(c[j].val) < 0.01) j++;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans max-w-md mx-auto shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium flex items-center gap-1">
                ...
                {new Date(currentGroup?.date).toLocaleDateString()}
              </p>
            </div>
          </div>
        ) : (
        ...
        <div className="flex items-center gap-2">
          {currentView === "GROUPS" && (
            <button
              ...
              className="text-blue-600 font-bold bg-blue-50 px-3 py-1.5 rounded-lg"
            >
              {t("newActivityBtn")}
            </button>
          )}
              ...
              groups.map((g) => {
                ...
                return (
                  <div
                    ...
                    className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 cursor-pointer active:scale-[0.98] transition-transform relative group"
                  >
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="flex gap-1">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setShowGroupDeleteConfirm(g.id);
                          }}
                ...
                {editingUser === u ? (
                  <div className="flex-1 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        className="flex-1 border-b p-1 text-sm font-mono outline-none"
                        value={editUserPayIdInput}
                        onChange={(e) => setEditUserPayIdInput(e.target.value)}
                        placeholder={t("enterPayId")}
                      />
                    </div>
                  ...
                  <>
                    <div>
                      <div className="font-medium text-gray-700 flex items-center gap-2">
                        {u}{" "}
                        {payIds[u] && (
                          <span className="text-[10px] bg-gray-100 px-1 rounded border">
            ...
            {activeTab === "expenses" && (
              <div className="p-4 space-y-4">
                {currentGroupExpenses.map((e) => (
                  <div
                    key={e.id}
                    className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center active:scale-[0.99] transition-transform cursor-pointer"
                    onClick={() => {
            ...
            {activeTab === "report" && (
              <div className="p-4 space-y-6">
                <div className="bg-blue-600 p-6 rounded-2xl text-white shadow-lg">
                  <p className="text-xs opacity-80 font-bold uppercase mb-1">
                    {t("totalSpending")}
                  </p>
                  <h2 className="text-4xl font-extrabold">
                    ${report.totalSpending.toFixed(1)}
                  </h2>
                </div>
                <div>
                  ...
                  <div className="space-y-3">
                    {report.tx.length === 0 ? (
                      ...
                      report.tx.map((txItem, i) => (
                        <div
                          key={i}
                          className="bg-white p-4 rounded-xl shadow-sm flex flex-col gap-2 border border-gray-100"
                        >
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2 font-bold text-gray-700">
                              <span>{txItem.from}</span>
                          ...
                          {txItem.payId && (
                            <div
                              className="bg-gray-50 p-2 rounded flex justify-between items-center text-xs text-gray-500 font-mono cursor-pointer hover:bg-gray-100"
                              onClick={() => copyToClipboard(txItem.payId)}
                            >
                  ...
                  <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                    {users.map((u) => {
                      ...
                      return (
                        <div
                          key={u}
                          className="p-4 border-b last:border-0 flex justify-between items-center border-gray-100"
                        >
                          <span className="font-medium text-gray-700">{u}</span>
                          <div className="text-right">
                            <div className="text-xs text-gray-400 mb-1">
                              {t("paid")} ${s.paid.toFixed(0)} / {t("share")} $
                              {s.share.toFixed(0)}
                            </div>
                            <div
                              className={`font-bold ${
      ...
      {/* Footer */}
      <div className="bg-white border-t px-6 py-2 flex justify-around items-center pb-safe z-30 border-gray-200">
        {currentView === "GROUP_DETAIL" ? (
          <>
            <button
              onClick={() => setActiveTab("expenses")}
              className={`flex flex-col items-center gap-1 w-16 ${
                activeTab === "expenses" ? "text-blue-600" : "text-gray-400"
              }`}
            >
              <Receipt className="w-6 h-6" />
              <span className="text-[10px] font-bold">{t("tabExpenses")}</span>
            </button>
            ...
            {activeTab === "expenses" && (
              <button
                ...
                className="absolute bottom-24 right-5 bg-blue-600 text-white w-20 h-20 rounded-full shadow-xl flex items-center justify-center active:scale-95 transition-transform hover:bg-blue-700 z-50"
              >
            ...
            >
              ...
              <span className="text-[10px] font-bold">{t("tabReport")}</span>
            </button>
          </>
        ) : (
          <>
            <button
              ...
              className={`flex flex-col items-center gap-1 w-20 ${
                currentView === "GROUPS" ? "text-blue-600" : "text-gray-400"
              }`}
            >
              <ShoppingBag className="w-6 h-6" />
              <span className="text-[10px] font-bold">{t("tabActivity")}</span>
            </button>
            <button
              onClick={() => setCurrentView("FRIENDS")}
              className={`flex flex-col items-center gap-1 w-20 ${
                currentView === "FRIENDS" ? "text-blue-600" : "text-gray-400"
              }`}
            >
              <Users className="w-6 h-6" />
              <span className="text-[10px] font-bold">{t("tabMembers")}</span>
            </button>
          </>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl transform scale-100">
            <h2 className="text-xl font-bold text-gray-800 mb-4">
              {editingGroupId ? t("editActivity") : t("createActivity")}
            </h2>
            <div className="mb-4">
              ...
              <input
                className="w-full border-b-2 border-blue-100 focus:border-blue-500 outline-none text-lg py-2"
                value={groupTitleInput}
                onChange={(e) => setGroupTitleInput(e.target.value)}
                placeholder={t("exampleActivity")}
                autoFocus
              />
            </div>
            <div className="mb-6">
              ...
              <input
                type="date"
                className="w-full bg-gray-50 rounded-lg px-3 py-2 outline-none border border-gray-100"
                value={groupDateInput}
                onChange={(e) => setGroupDateInput(e.target.value)}
              />
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setIsGroupModalOpen(false)}
                className="flex-1 py-3 text-gray-500 font-bold bg-gray-100 hover:bg-gray-200 rounded-xl"
              >
                {t("cancel")}
              </button>
              <button
                onClick={saveGroup}
                className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-md active:scale-95 transition-transform"
              >
                {t("save")}
              </button>
            </div>
      ...
      {viewingImage && (
        <div
          className="fixed inset-0 bg-black z-[70] flex items-center justify-center p-4 animate-in fade-in duration-200"
      ...
      {showGroupDeleteConfirm && (
        <ConfirmDialog
          ...
          onCancel={() => setShowGroupDeleteConfirm(null)}
          t={t}
        />
      )}
